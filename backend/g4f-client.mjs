...
// ============ NOVOS PROVIDERS GRATUITOS (descobertos no GitHub g4f) ============

// NOTA: Muitos providers do Python G4F usam scraping avançado que não funciona
// em ambiente servidor Node.js. Apenas providers com API direta funcionam.

// Groq - Free tier (rápido!) - REQUER API KEY GRATUITA
class Groq extends Client {
    constructor(options = {}) {
        if (!options.apiKey) {
    ...
    get models() {
        return {
            list: async () => {
                return [
                    { id: "llama-3.3-70b-versatile", name: "Llama 3.3 70B (Groq)", type: "chat", speed: "very-fast" },
                    { id: "llama-3.1-70b-versatile", name: "Llama 3.1 70B (Groq)", type: "chat", speed: "very-fast" },
                    { id: "llama-3.1-8b-instant", name: "Llama 3.1 8B Instant", type: "chat", speed: "ultra-fast" },
...
// Cerebras - Free tier com modelos rápidos - REQUER API KEY GRATUITA
class Cerebras extends Client {
    constructor(options = {}) {
        if (!options.apiKey) {
    ...
    get models() {
        return {
            list: async () => {
                return [
                    { id: "llama3.1-70b", name: "Llama 3.1 70B (Cerebras)", type: "chat", speed: "fast" },
                    ...
                    { id: "deepseek-r1-distill-llama-70b", name: "DeepSeek R1 Distill 70B", type: "chat", speed: "fast" },
                ];
            }
        };
    }
...
// OpenRouter Free - Usa modelos gratuitos do OpenRouter - REQUER API KEY GRATUITA
class OpenRouterFree extends Client {
    constructor(options = {}) {
        if (!options.apiKey) {
    ...
    get models() {
        return {
            list: async () => {
                // Retorna apenas modelos gratuitos populares
                return [
                    { id: "google/gemini-2.0-flash-exp:free", name: "Gemini 2.0 Flash (Free)", type: "chat" },
                    { id: "google/gemma-2-9b-it:free", name: "Gemma 2 9B (Free)", type: "chat" },
                    { id: "meta-llama/llama-3.2-3b-instruct:free", name: "Llama 3.2 3B (Free)", type: "chat" },
                    { id: "microsoft/phi-3-mini-128k-instruct:free", name: "Phi 3 Mini (Free)", type: "chat" },
                    { id: "qwen/qwen-2-7b-instruct:free", name: "Qwen 2 7B (Free)", type: "chat" },
                ];
            }
        };
    }
